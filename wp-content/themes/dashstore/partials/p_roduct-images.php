	...
	function dash_get_attachment_imgs($size, $main_slider, $slider_type){
		...
		if ($attachment_ids) {
			foreach ( $attachment_ids as $attachment_id ) {
				...
				if ($main_slider) {
					$gallery_imgs .= apply_filters( 'woocommerce_single_product_image_thumbnail_html', sprintf( '<a href="%s" itemprop="image" class="woocommerce-main-image" title="%s" >%s</a>', $image_link, $image_title, $image ), $attachment_id );
				} else {
