...
if ( class_exists('WCV_Vendors') ) {
	...
	function dash_add_backend_vendor_fields($user) { ?>
	  ...
	  <h3><?php esc_html_e( 'Extra Vendor Options (Dash Store Modifications)', 'dashstore' ); ?></h3>
	...
	function dash_add_frontend_vendor_fields() { ?>
	  ...
	  <?php $user_id = get_current_user_id(); ?>
		...
		<h2 class="settings-title"><?php esc_html_e( 'Extra Vendor Options (Dash Store Modifications)', 'dashstore' ); ?></h2>
	...
	function dash_add_frontend_vendor_pro_fields() {
		$user_id = get_current_user_id(); ?>
			...
			<hr style="clear: both;" />
			<h2><?php _e('Handy Store extra Settings', 'dashstore'); ?></h2>
			<div class="pv_featured_carousel_container">
		    <p><strong><?php esc_html_e( 'Products Carousel', 'dashstore' ); ?></strong></p>
		    <?php $value = get_user_meta( $user_id,'pv_featured_carousel', true ); ?>
		    <p>
	...
	function dash_add_frontend_vendor_pro_product_fields( $object_id ) { ?>
		...
		<div class="all-100">
				...
				<input type="checkbox" class="input-checkbox" name="pt_product_extra_gallery" id="pt_product_extra_gallery" <?php checked( $pt_product_extra_gallery[0], 'on', true ); ?> /><label class="checkbox" for="pt_product_extra_gallery"><?php _e( 'Use extra gallery for this product', 'dashstore' ) ?></label>
				<p><?php _e( 'Check the checkbox if you want to use extra gallery (appeared on hover) for this product. The first 3 images of the product gallery are going to be used for gallery.', 'dashstore'); ?></p>
			</div>
			<br />
			<div class="vendor-special-offers-carousel">
				<input type="checkbox" class="input-checkbox" name="pt_vendor_special_offers_carousel" id="pt_vendor_special_offers_carousel" <?php checked( $pt_vendor_special_offers_carousel[0], 'on', true ); ?> /><label class="checkbox" for="pt_vendor_special_offers_carousel"><?php _e( 'Add this product to "Special Offers" carousel', 'dashstore' ) ?></label>
				<p><?php _e( 'Check the checkbox if you want to add this product to the "Special Offers" carousel on your Vendor Store Page.', 'dashstore'); ?></p>
			</div>
	...
	function dash_save_new_vendor_pro_fields( $user_id ) {
		if ( isset( $_POST['pv_featured_carousel'] ) ) {
		    update_post_meta( $user_id, 'pv_featured_carousel', $_POST['pv_featured_carousel'] );
		} else {
		  	update_post_meta( $user_id, 'pv_featured_carousel', 'off' );
		}
		if ( isset( $_POST['pv_question_form'] ) ) {
		    update_post_meta( $user_id, 'pv_question_form', $_POST['pv_question_form'] );
		} else {
		  	update_post_meta( $user_id, 'pv_question_form', 'off' );
		}
	}
